...
if [[ $1 == "GENERAL" ]];then
   #ulist="rpa sfg prd-dde-eksctl"  eksctl never had an access token , so it probably not required
   ulist="rpa sfg"
fi
if [[ $1 == "GIT" ]];then
   ulist="prd-dde-integration"
...
if [[ $1 == "CLI" ]];then
   ulist="prd-dde-programmatic-access"
fi
...
if [[ "A"$ulist == "A" ]];then
   echo
   echo "Command: DDE-rotate.sh < PROD | GIT | CLI >"
   echo
...
"https://gitlab.com/api/v4/projects/$ACCNT_VC/variables/AWS_SECRET_ACCESS_KEY?filter\[environment_scope\]=prod"
	    ...
	    if [[ $err1 == 0 && $err2 == 0 ]];then
              ...
              echo "There was an error during the secret updated" | tee -a ~/keys/status.log
	      hostname=`hostname`
	      echo "kubectl patch secret:     err1=$err1     err2=$err2" | mutt -s "$hostname kubectl secret updates failed: Please investigate" -- "arnulf.hanauer@vcontractor.co.za"
	    fi  
           ;;
