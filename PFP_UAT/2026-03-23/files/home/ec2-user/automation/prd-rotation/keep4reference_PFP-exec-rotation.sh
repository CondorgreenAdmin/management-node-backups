...
do
  ...
  "https://api.github.com/repos/$OWNER/$REPO/actions/secrets/public-key")
               ...
               ENCODED_SECRET_NAME=$(echo -n "$NEW_SECRET_NAME_VALUE" | \
  openssl pkeyutl -encrypt -pubin -inkey <(echo "$PUBLIC_KEY" | base64 -d) -out /dev/stdout | base64 -w 0)
               ENCODED_SECRET_VALU=$(echo -n "$NEW_SECRET_VALU_VALUE" | \
  openssl pkeyutl -encrypt -pubin -inkey <(echo "$PUBLIC_KEY" | base64 -d) -out /dev/stdout | base64 -w 0)

               echo "Updating CG GitHub ACCESS_KEY" | tee -a ~/keys/status.log

               curl -v -s -X PUT -H "Authorization: token $PVTOKEN_CG" \
  -H "Content-Type: application/json" \
  -d "{\"encrypted_value\":\"$ENCODED_SECRET_NAME\", \"key_id\":\"$KEY_ID\"}" \
  "https://api.github.com/repos/$OWNER/$REPO/actions/secrets/$SECRET_NAME" | tee -a ~/keys/status.log
	       RES1=$?
  ...
  "https://api.github.com/repos/$OWNER/$REPO/actions/secrets/$SECRET_VALU" | tee -a ~/keys/status.log
	       RES2=$?
