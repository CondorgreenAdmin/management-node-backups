# Management Node Backup -> GitHub
#
# Backs up (readable only):
#   - All *.sh scripts
#   - Config files by extension (conf/cfg/yaml/json/env/etc)
...
#
# IMPORTANT:
# - "latest/" is the always-kept snapshot. Even if a file doesn't change for
#   longer than the retention window, it still remains in latest/.
# - Dated folders are a "changes-only" history and will be pruned by retention.
#-----------------------------------------------------------------------------
...
# Print error if token file does not exist and exit
...
# Turn BACKUP_STORE into environment variable
set -a
...
# 2) FIXED SETTINGS
#-------------------
# ~3 months retention for dated snapshots
RETENTION_DAYS=90
REPO_DIR="${SCRIPT_DIR}/repo"
LOG_DIR="${SCRIPT_DIR}/logs"
LOG_FILE="${LOG_DIR}/github_backup.log"

# Search from "/" so we include ALL folders on the node.If new folders are added anywhere, they are automatically covered.
SEARCH_ROOTS=("/")

# Everything that counts as a config file (Can be edited)
CONFIG_EXTS=("conf" "cfg" "cnf" "ini" "properties" "yaml" "yml" "json" "toml" "env")

# Exclude big/noisy system trees + temp/log/cache + anything that creates a lot of
# "config-like" files that are not relevant
# NOTE: Keep "$REPO_DIR" excluded so we never back up the repo working copy.
# This can be edited
EXCLUDE_PATHS=(
  "/proc" "/sys" "/dev" "/run"

  # Big/noisy system trees (not your node config/scripts)
  "/bin" "/sbin" "/lib" "/lib64"
  ...
  # secrets
  "/home/ec2-user/.config/gh/*"

)

# Pattern-based excludes inside other folders (e.g. /home/ec2-user/...).
# This avoids backing up dependency/vendor/cache folders that contain tons of
# .json/.ini/.env-like files.
EXCLUDE_PATTERNS=(
  "*/.git/*"
  "*/node_modules/*"
  "*/.terraform/*"
...
# 3) BASIC VALIDATION
#-----------------------
# Check  if git and sha2456sum is availble on node
...
mkdir -p "$LOG_DIR"
# Write to stdout (screen) and appends to log file
...
# Used to determine whether a file changed
...
DATE_STAMP="$(date +%F)"
# Prevent overwriting a dated backup
...
# Convert an absolute OS path into the repo's files/.. path. Ensure full paths are backed up
relpath_from_abs() {
  local abs="$1"
  abs="${abs#/}"
  echo "files/$abs"
...
# Computes sha-256 hash of a file's contents and returns the hash string
file_hash() {
  sha256sum "$1" | awk '{print $1}'
}

# prevents token and secret files from being backed up
is_secret_file() {
  case "$1" in
...
# Build a "prune" expression so find does NOT traverse excluded trees.
# This keeps the "all folders" requirement practical (fast + less noise).
# It supports:
#   - Absolute excludes (EXCLUDE_PATHS) + their children
#   - Pattern excludes (EXCLUDE_PATTERNS) anywhere in the tree
build_prune_expr() {
  local -a with_children=()
  local first=1

  # Absolute paths + children
  for ex in "${EXCLUDE_PATHS[@]}"; do
    if [[ "$first" -eq 1 ]]; then
      with_children+=( -path "$ex" -o -path "$ex/*" )
      first=0
    else
      with_children+=( -o -path "$ex" -o -path "$ex/*" )
    fi
  done

  # Pattern prunes (these already include wildcards)
  for pat in "${EXCLUDE_PATTERNS[@]}"; do
    if [[ "$first" -eq 1 ]]; then
      with_children+=( -path "$pat" )
      first=0
    else
      with_children+=( -o -path "$pat" )
    fi
  done
...
# 7) RETENTION
#--------------------
# Delete only when `find` actually matches an old folder (using -print -quit).
retention_deleted=0
if [[ -d "$ENV_DIR" ]]; then
  ...
  while IFS= read -r d; do
    ...
    if [[ "$base" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}(_[0-9]{6})?$ ]]; then
      # Only delete if the directory is older than RETENTION_DAYS
      if [[ -n "$(find "$d" -maxdepth 0 -type d -mtime +"$RETENTION_DAYS" -print -quit 2>/dev/null)" ]]; then
...
# Search from "/" (all folders), but prune excluded trees + patterns.
for root in "${SEARCH_ROOTS[@]}"; do
  [[ -d "$root" ]] || continue

  # Build prune args into an array
  PRUNE_ARGS=()
  while IFS= read -r tok; do
    PRUNE_ARGS+=( "$tok" )
  done < <(build_prune_expr)

  # Find *.sh files
  find "$root" \
    \( "${PRUNE_ARGS[@]}" \) -prune -o \
    -type f -name "*.sh" -print 2>/dev/null >> "$TMP_LIST" || true

  # Find config extension files
  for ext in "${CONFIG_EXTS[@]}"; do
    find "$root" \
      \( "${PRUNE_ARGS[@]}" \) -prune -o \
      -type f -iname "*.${ext}" -print 2>/dev/null >> "$TMP_LIST" || true
  done
done
...
# 9) COLLECT CRONS (Not all are readable)
#----------------------------------------
