...
profile="nonprod_pfp_cli_access"

# List all IAM users and their access key IDs, statuses, and ages
users=$(aws iam list-users --query 'Users[*].UserName' --output text --profile $profile)

for user in $users; do
        # List all access keys (active and inactive) for each user
        access_keys=$(aws iam list-access-keys --user-name "$user" --query 'AccessKeyMetadata[*].[AccessKeyId, Status, CreateDate]' --output text --profile $profile)
