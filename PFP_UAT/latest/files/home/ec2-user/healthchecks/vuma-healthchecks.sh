...
#DL="arnulf.hanauer@vcontractor.co.za, michaelalex.dirks@vcontractor.co.za"
DL=$(</home/ec2-user/paths/EMAIL_CG_SUPPORT)

DTE1=$(date +"%d-%B-%Y")
DTE2=$(date +"%Y-%m-%d")

attachment=$(echo "VUMA_NONPROD(UAT)_healthcheck_"$DTE2".html")
out1=$$_temp_html

#start with HTML headers
echo "VUMA HEALTHCHECKS" > $out1
...
###Publish report
mutt -e "set content_type=text/html" -s "VUMA NONPROD(UAT) Health Checks" -a ${attachment} -- $DL < $attachment
