...
# NOTES:
#   - Cron runs with a much smaller environment than your normal shell, so
#     we explicitly set the basic environment variables needed by kubectl.
...
# File used to store the real kubectl error output when Step 1 fails
ERROR_FILE="$WORK_DIR/kubectl_error.txt"
...
# Full path to kubectl
# Using the full path is safer in cron because cron PATH is often limited
KUBECTL_BIN="/usr/local/bin/kubectl"

# CRON ENVIRONMENT SETUP
# ----------------------
# Cron does not always load your normal shell profile, so set the important
# variables here to make sure kubectl can run the same way every time.
...
# Use full kubectl path so cron does not fail because of PATH differences.
# Send stdout to the normal pods file.
# Send stderr to a separate error file so the real problem can be included
# in the email if this step fails.
"$KUBECTL_BIN" get pods -A \
  ...
  --no-headers > "$PODS_FILE" 2> "$ERROR_FILE"
...
do
  ...
  # Get pod description
  # ----------------------------------------------------------------------------------------
  describe_output=$("$KUBECTL_BIN" describe pod "$pod_name" -n "$namespace" 2>&1)

  # ----------------------------------------------------------------------------------------
  # Get current logs
  # This may fail for some pods, which is okay - we include the error text in the email
  # ----------------------------------------------------------------------------------------
  current_logs=$("$KUBECTL_BIN" logs "$pod_name" -n "$namespace" --tail="$LOG_LINES" 2>&1)

  # ----------------------------------------------------------------------------------------
  # Get previous logs
  # This is useful for restarted containers such as CrashLoopBackOff pods
  # ----------------------------------------------------------------------------------------
  previous_logs=$("$KUBECTL_BIN" logs "$pod_name" -n "$namespace" --previous --tail="$LOG_LINES" 2>&1)
