...
# Without prune, find will still walk through /proc, /sys, /var/lib, etc (slow/noisy),
# even though we later skip files. This keeps the "all folders" requirement practical.
build_prune_expr() {
  local -a expr=()
  local first=1
  for ex in "${EXCLUDE_PATHS[@]}"; do
    # If EXCLUDE_PATHS contains dynamic values (like $WORKDIR), they are already expanded.
    if [[ "$first" -eq 1 ]]; then
      expr+=( -path "$ex" )
      first=0
    else
  ...
  # Match excluded dir itself AND everything under it
  # e.g. -path /proc -o -path /proc/*
  ...
  # Return the prune expression as an array via echo with delimiter is messy,
  # so we print one per line and read it into an array where needed.
  printf '%s\n' "${with_children[@]}"
}
...
# CHANGED (MINIMAL + REQUIRED):
# Instead of searching only specific roots, search from "/" (all folders),
# and use -prune so excluded directories are not traversed.
#
# We collect:
#   - all *.sh
#   - all files matching config extensions
#
# Output goes to TMP_LIST, then we sort -u.
