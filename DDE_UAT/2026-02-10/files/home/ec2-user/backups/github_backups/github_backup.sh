...
# "config-like" files that are not relevant to your management node scripts/config.
# NOTE: Keep "$WORKDIR" excluded so we never back up the repo working copy.
EXCLUDE_PATHS=(
  "/proc" "/sys" "/dev" "/run"
...
########################################
# FIXED:
# The previous version used `if find ...; then`, but `find` returns exit code 0
# even when it finds nothing (unless there's an error). That caused it to delete
# ALL dated snapshot folders every run.
#
# We now delete only when `find` actually matches an old folder (using -print -quit).
retention_deleted=0
if [[ -d "$ENV_DIR" ]]; then
...
#   cron user.
# - Token format example in github_backup.env:
#     BACKUP_STORE=github_pat_11xxxC7_txxx2
