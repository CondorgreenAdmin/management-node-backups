...
LOGFILE="./logs/output_logs_$(date +%Y%m%d_%H%M).log"
...
then
	...
	aws s3 mv s3://dev-dopadde-share/sp_deployments/ s3://dev-dopadde-share/sp_deployments_archive/$version/ --recursive
	echo "Cleaned up S3"
	echo "Starting to clean local"
	mv downloaded "downloaded_${date_time}"
	mv rollback_scripts "./rollback_scripts_archive/rollback_scripts_${date_time}"
	echo "Local cleaned"
fi
...
echo "Please find logs attached for the SP deployments :-)" | mutt -s "Log File - UAT - $(date +%Y%m%d_%H%M)" -a "$LOGFILE" -- "$DL"
