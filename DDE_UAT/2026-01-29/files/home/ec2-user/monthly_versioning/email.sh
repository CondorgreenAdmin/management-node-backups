...
#DL="michaelalex.dirks@vcontractor.co.za, michael.dirks@condorgreen.com"
...
echo "DDE Application & Database Procudure versioning" > $out1
echo "NONPROD(UAT)" >> $out1
cr_webpage_begin $out1 $attachment


cr_heading "DDE Application Software Versioning"
echo "Date Image Pod_Full_Name Published_version Running_version" > heading.txt
#./show_app_versions.sh >> myfile
cr_current_block_column_compare heading.txt ~/monthly_versioning/show_app_version_monthly.txt "4" "5"

cr_heading "DDE Database Stored Procedure & Functions Versioning"
echo "Date Object Release_Version Published_timestamp Published_Tracking_No Running_timestamp Running_Tracking_No" > heading.txt
#./show_proc_versions.sh >> myfile
cr_current_block_column_compare heading.txt ~/monthly_versioning/show_proc_version_monthly.txt "5" "7"
...
#mutt -e "set content_type=text/html" -s "DDE NONPROD(UAT) Monthky Release Checks" -a ${attachment} lukhanyo.vakele@vcontractor.co.za < $attachment


#ls -lt | head -10
#echo
#echo
rm heading.txt show_proc_version_monthly.txt show_app_version_monthly.txt $attachment $out1
#echo
#ls -lt | head -10
