...
export INFORMIXSERVER=csgeppixqa_odbc

#DL="arnulf.hanauer@vcontractor.co.za, yusuf.pinn@vcontractor.co.za"
DL="lukhanyo.vakele@vcontractor.co.za, Ihsaan.deVilliers@condorgreen.com, olwethu.ketwa@condorgreen.com"

cd ~/automation/dataloads/scripts

./unload-raw-siebel-vas.sh
rc=$?

if [ $rc -eq 0 ];then
	sleep 3
	./transform-raw-siebel-vas.sh
	if [ $? -eq 0 ];then

		sleep 3
		./load-mysql-raw-siebel-vas.sh
		if [ $? -ne 0 ];then
     			logger "ERROR LOADING: sending email"
			tail -10 logs/status.log > $$_msg
			mutt -s "DDE Infinity refresh has failed during LOADING - Please investigate" -- $DL < $$_msg
		else
			tail -10 logs/status.log > $$_msg
			mutt -s "DDE Infinity source refresh succeeded" -- $DL < $$_msg	
		fi	
	else
     		logger "ERROR TRANSFORMING: sending email"
		tail -10 logs/status.log > $$_msg
		mutt -s "DDE Infinity source refresh has failed during TRANSFORMATION - Please investigate" -- $DL < $$_msg	
	fi
else
     logger "ERROR UNLOADING: sending email"
     tail -10 logs/status.log > $$_msg
     mutt -s "DDE Infinity source refresh has failed during UNLOAD - Please investigate" -- $DL <  $$_msg
