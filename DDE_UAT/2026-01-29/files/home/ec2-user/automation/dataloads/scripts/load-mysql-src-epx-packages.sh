...
mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "TRUNCATE src_epx_packages_backup;" --batch --raw --local-infile=1
...
NUM_SRC=$(mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "select count(*) from src_epx_packages" --batch --raw --skip-column-names)
logger "	Found $NUM_SRC records"


logger "Initiate MySQL backup insert : src_epx_packages_backup"

mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "INSERT INTO src_epx_packages_backup SELECT * FROM src_epx_packages" --batch --raw --local-infile=1
...
NUM_BACKUP=$(mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "select count(*) from src_epx_packages_backup" --batch --raw --skip-column-names)
...
if (( $NUM_SRC == $NUM_BACKUP ));then
   ...
   mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "TRUNCATE src_epx_packages;" --batch --raw --local-infile=1
   ...
   mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "LOAD DATA LOCAL INFILE 'data/cleaned_vpk_package.csv' INTO TABLE src_epx_packages FIELDS TERMINATED BY '|' ENCLOSED BY '\"' LINES TERMINATED BY '\n';" --batch --raw --local-infile=1
   ...
   NUM_NEWSRC=$(mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "select count(*) from src_epx_packages" --batch --raw --skip-column-names)
