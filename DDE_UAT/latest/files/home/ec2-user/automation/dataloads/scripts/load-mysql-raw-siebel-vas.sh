...
# TRUNCATE
logger "Initiate MySQL table truncate : raw_infinity_vas"

mysql --defaults-extra-file=./dde-uat-admin.cnf --defaults-group-suffix=1 -e "TRUNCATE raw_siebel_vas;" --batch --raw --local-infile=1

rc=$?
if (( $rc == 0 ));then
   logger "Successful MySQL truncate : raw_infinity_vas"
else
   logger "Error during MySQL truncate : raw_infinity_vas"
   exit $rc
fi

# INSERT
logger "Initiate MySQL table insert : raw_infinity_vas"
...
rc=$?
if (( $rc == 0 ));then
   logger "Successful MySQL data loaded : raw_infinity_vas"
else
   logger "Error during MySQL data load : raw_infinity_vas"
   exit $rc
fi
...
logger "Total records available in [ data/infinity_vas_cleaned.csv ]: $NUM_FILE"
logger "Total records loaded to table [ raw_infinity_vas ]: $NUM_NEWSRC"
