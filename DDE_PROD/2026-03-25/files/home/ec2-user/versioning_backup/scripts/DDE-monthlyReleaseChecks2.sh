...
# Testing section, uncomment below
...
# Uncomment line below to get the current month's version 
#PREV_DATE=$(date -d 'today' +%m_%Y)

attachment=$(echo "DDE_PROD_monthly_release_checks_"$DTE2".html")
out1=$$_temp_html
...
cr_heading "DDE Application Software Versioning for Client Application"
echo "Date Image Pod_Full_Name Published_version Running_version CRQ" > heading.txt
cat ~/versioning/scripts/software_versioning_${PREV_DATE}.txt | grep client-depl > $$_sort_data
cr_current_block_column_compare heading.txt $$_sort_data  "4" "5"

cr_heading "DDE Application Software Versioning for API"
echo "Date Image Pod_Full_Name Published_version Running_version CRQ" > heading.txt
cat ~/versioning/scripts/software_versioning_${PREV_DATE}.txt | grep dde-depl > $$_sort_data
cr_current_block_column_compare heading.txt $$_sort_data  "4" "5"

cr_heading "DDE Application Software Versioning for Loader Application"
echo "Date Image Pod_Full_Name Published_version Running_version CRQ" > heading.txt
cat ~/versioning/scripts/software_versioning_${PREV_DATE}.txt | grep loader-depl > $$_sort_data
cr_current_block_column_compare heading.txt $$_sort_data  "4" "5"

cr_heading "DDE Application Software Versioning for Worker Application"
echo "Date Image Pod_Full_Name Published_version Running_version CRQ" > heading.txt
cat ~/versioning/scripts/software_versioning_${PREV_DATE}.txt | grep worker-depl > $$_sort_data
cr_current_block_column_compare heading.txt $$_sort_data  "4" "5"
...
filename=$(echo "DDE_PROD_monthly_release_checks_"$DTE2".html")
cat $attachment > ~/versioning/scripts/monthly_release/$filename

###Publish report
mutt -e "set content_type=text/html" -s "DDE PROD Monthly Release Checks" -a ${attachment} -- $DL < $attachment
#mutt -e "set content_type=text/html" -s "DDE PROD Health Checks" arnulf.hanauer@vcontractor.co.za < $attachment
