...
do
        ...
        mysql --defaults-extra-file=dde-sox.cnf --comments < $file
        error=$?
...
then
        ...
        mysql --defaults-extra-file=dde-sox.cnf --comments < rollback.sql
...
humandate=$(mysql --defaults-extra-file=dde-sox.cnf --defaults-group-suffix=1 -e 'CALL util_get_recent_update("DDE-prd");' --batch --raw -s)
...
DL="michaelalex.dirks@vcontractor.co.za, sivuyile.sifuba@vcontractor.co.za, yusuf.pinn@vcontractor.co.za, arnulf.hanauer@vcontractor.co.za"
#DL="michaelalex.dirks@vcontractor.co.za"
echo "Please find logs attached for the SP deployments :-)" | mutt -s "DDE PROD SP_Deployment log file - $(date +%Y%m%d_%H%M)" -a "$LOGFILE" -- "$DL"
