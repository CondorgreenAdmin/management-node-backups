...
mysql --defaults-extra-file=$CNF --defaults-group-suffix=1 --batch  < ./vb_cycle.sql | sed 's/\t/|/g' | sed 's/NULL//g' > VB_template_cycle-used.csv
DC=$(tail -1 VB_template_cycle-used.csv)
...
aws s3 cp . s3://prd-dde-share/xls_ingestion/vb_transfers/ --recursive --exclude "*" --include "VB_template_*"
RC_S3=$?
...
echo -e "PRODUCTION VB templates data generated on $DTE. \n\nDeal cycle used: $DC\n\nReturn codes & counts were\n\nTariffs: RC: $RC_T    COUNT: $CNT_T \n\nDevices: RC: $RC_D     COUNT: $CNT_D \n\nAccessories: RC: $RC_A     COUNT: $CNT_A \n\nVases: RC: $RC_V     COUNT: $CNT_V \n\nAWS S3: RC: $RC_S3\n" | mutt -s "DDE(PROD) VB_template generated: $DTE" -- $DL 
