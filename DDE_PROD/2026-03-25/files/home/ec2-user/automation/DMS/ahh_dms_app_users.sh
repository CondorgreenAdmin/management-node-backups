...
echo "Data output follows:"
echo "================================================="
cat $$TEMP
echo "======="
cat $$TEMP | grep hanauera
echo "================================================="
...
echo curl --cacert /home/ec2-user/automation/DMS/dms2.pem --request POST --url https://dms.vodacom.corp:8443/dms/uploadfile --header 'Content-Type: multipart/form-data' --header 'Authorization:Basic c3ZjX2RkZTowOTEzU21pbGUmQA==' --form "file=@/home/ec2-user/automation/DMS/$OUT_CSV"
...
EOF
		     ...
		     echo "$DTE : User $EMAIL was notified of $AGE remaining days" >> history/test_dormancy.log
		   ;;
...
EOF
                     echo mysql --defaults-extra-file=$CNF --defaults-group-suffix=1 --batch  -e "update usr_users set is_deleted=1 where email=\"$EMAIL\""  
                     #####mutt -s "DDE(Production) User dormancy notification" -- $EMAIL < $$outMSG
                     #####mutt -s "DDE(Production) User dormancy notification: $EMAIL" -- $DL < $$outMSG
		     echo "$DTE : User $EMAIL was notified of account deletion. DDE support also notified ($DL)" >> history/test_dormancy.log
		   ;;
		esac
	fi
