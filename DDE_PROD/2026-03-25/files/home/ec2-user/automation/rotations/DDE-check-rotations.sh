...
for user in rpa sfg prd-dde-integration prd-dde-programmatic-access prd-dde-eksctl
do
   USER_NAME=$user
   ...
   if [ $ddiff -gt 84 ]; then
      ...
      rc=$?
     if (( $rc != 0 )); then
         #echo "AWS key rotation has failed for user $USER_NAME" | mutt -s "ERROR: AWS keys rotation [$USER_NAME] failed in the DDE PROD environment" -- $DL
         echo "AWS key rotation has failed for user $USER_NAME"
      else
         #echo "AWS key age for user $USER_NAME was $ddiff" | mutt -s "AWS keys rotation executed [$USER_NAME] for the DDE PROD environment" -- $DL
