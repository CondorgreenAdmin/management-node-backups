...
if [[ $USR == "prd-dde-programmatic-access" ]];then
   OPT="CLI"
fi
if [[ $USR == "rpa" ]];then
    OPT="rpa"
...
case $OPT in
     "CLI")
        ...
        echo "prd-dde-programmatic-access rotated" > "$temp_file"
       ;;
     "GIT")
        OUT="/home/ec2-user/.aws/credentials"
        PROFILE="prd-dde-integration"
        TMP="/tmp/credentials.tmp"
        DTE2=$(date +"%Y%m%d-%H%M%S")
      ...
      "https://gitlab.com/api/v4/projects/$ACCNT_VC/variables/AWS_SECRET_ACCESS_KEY?filter\[environment_scope\]=prod"
        ...
        "masked": true,
        "environment_scope": "prod"}' \
        "https://gitlab.com/api/v4/projects/$ACCNT_CG_DB/variables/AWS_ACCESS_KEY_ID?filter\[environment_scope\]=prod"
        err5=$?
        ...
        "masked": true,
        "environment_scope": "prod"}' \
        "https://gitlab.com/api/v4/projects/$ACCNT_CG_DB/variables/AWS_SECRET_ACCESS_KEY?filter\[environment_scope\]=prod"
        err6=$?
        ...
        sumerr=$(( err1 + err2 + err3 + err4 + err5 + err6 ))
         ...
         echo "$PROFILE updated" | tee -a ~/keys/status.log
         echo "prd-dde-integration rotated" > "$temp_file"
       ;;
     "rpa")
        OUT="/home/ec2-user/.aws/credentials"
        PROFILE="rpa"
     ...
     "eksctl")
        ...
        echo "Secrets Manager updated with new AWS credentials for $PROFILE." | tee -a ~/keys/status.log
        echo "prd-dde-eksctl rotated" > "$temp_file"
        ;;
esac
...
{
    echo "DDE PROD Keys rotated. Please see below for details."
    echo
    cat "$temp_file"
} | mutt -s "DDE PROD - Key rotation" -a "$temp_file" -- $DL
