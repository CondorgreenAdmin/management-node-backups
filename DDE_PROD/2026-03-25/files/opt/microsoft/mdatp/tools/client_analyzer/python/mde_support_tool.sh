...
# If in venv, install without --user (goes to venv); otherwise use --user (goes to PYTHONUSERBASE/.deps)
if [[ -z "$VIRTUAL_ENV" ]]; then
    PIP_USER_FLAG="--user"
    INSTALL_LOCATION="$PYTHONUSERBASE"
else
    PIP_USER_FLAG=""
    INSTALL_LOCATION="virtual environment ($VIRTUAL_ENV)"
fi
...
if [ $? = $PACKAGE_MISSING_EXIT_CODE ]; then
    echo "installing dependencies to $INSTALL_LOCATION (Nothing is installed system-wide)..."
    ...
    if ! $PYTHON_ -m pip --version &> /dev/null; then
        ...
        $PYTHON_ "$PYTHONUSERBASE/get-pip.py" $PIP_USER_FLAG
    fi

    while IFS= read -r dep; do
        $PYTHON_ -m pip install $PIP_USER_FLAG "$dep" || exit_failure "Failed to install $dep in mde_tools directory"
    done < "$DIR"/requirements.txt

    echo -e "\ninstalling optional dependencies to $INSTALL_LOCATION (Nothing is installed system-wide)..."
    while IFS= read -r dep; do
        $PYTHON_ -m pip install $PIP_USER_FLAG "$dep" 2>/dev/null || echo "[WARNING] Unable to install optional dependency $dep"
