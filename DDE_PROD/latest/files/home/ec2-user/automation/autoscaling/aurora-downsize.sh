...
LOG_FILE="$SCRIPT_DIR/aurora-autoscale.log"
...
if [[ "$WRITER_CLASS" != "$SCALE_CLASS" || "$READER_CLASS" != "$BASE_CLASS" ]]; then
  echo "Cluster not in expected HOT layout (Writer=$SCALE_CLASS, Reader=$BASE_CLASS). Aborting downsize."
  exit 1
fi
...
echo "Sleeping between failover"
sleep 15
