...
releaseInfo=$(cat ~/versioning/sp_deployments/sp_deployments_version_master | awk '{print $1,$3"#"$4,$2}')
...
if [[ $LAST_RUN_VERSION != $RUN_VERSION ]]; then
      ...
      CRQ="$(aws s3 cp s3://prd-dde-share/deployment_artifacts/CRQ.txt - | grep PROC | awk -F "=" '{print $2}')"
