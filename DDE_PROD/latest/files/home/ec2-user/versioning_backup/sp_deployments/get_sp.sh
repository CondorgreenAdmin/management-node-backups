...
#	echo '(Processing file: $(basename "$FILE")'
    ...
    IS_FUNCTION=$(mysql --defaults-file="$DEFAULTS_FILE" -D "$DB_NAME" -N -e \ "SELECT COUNT(*) FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE='FUNCTION' AND ROUTINE_NAME='$FUNCTION_OR_PROCEDURE_NAME' AND ROUTINE_SCHEMA='$DB_NAME';")
    ...
    elif [ "$IS_TRIGGER" -eq 1 ]; then echo "DROP TRIGGER IF EXISTS $FUNCTION_OR_PROCEDURE_NAME;" > "$BACKUP_DIR/$FUNCTION_OR_PROCEDURE_NAME.sql" else
	fi
...
#    echo "THIS IS THE FILE LOCATION - $FILE"
   ...
   if grep -iq 'CREATE PROCEDURE' "$FILE"; then
	...
	sed -n '/Create Procedure:/,/character_set_client:/p' | sed -e 's/^.*Create Procedure:[[:space:]]*//' -e '/character_set_client:/d'
