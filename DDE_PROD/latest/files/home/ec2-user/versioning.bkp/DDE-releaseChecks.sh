...
attachment=$(echo "DDE_PROD(TEST)_release_checks_"$DTE2".html")
out1=$$_temp_html
...
filename=$(echo "DDE_PROD(TEST)_release_checks_"$DTE2".html")
cat ${attachment} > ~/versioning/scripts/daily_release/$filename

###Publish report
mutt -e "set content_type=text/html" -s "DDE PROD(TEST) Release Checks" -a ${attachment} -- $DL < $attachment
#mutt -e "set content_type=text/html" -s "DDE PROD(TEST) Health Checks" arnulf.hanauer@vcontractor.co.za < $attachment
